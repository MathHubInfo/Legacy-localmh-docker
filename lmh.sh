...
function command_ensure_running(){
  ...
  if [ "$(docker inspect --format='{{ .State.Running }}' $docker_pid 2> /dev/null || echo 'no')" == "no" ]; then
    ...
    if [ "$lmh_devmode" == "true" ]; then
      docker_pid=$(docker create -h lmh_docker_dev -v "$lmh_sshdir:/root/.ssh" -v "$lmh_devdir:/path/to/localmh"  $lmh_repo )
    else
      docker_pid=$(docker create -h lmh_docker  -v "$lmh_sshdir:/root/.ssh" -v "$lmh_mountdir:/path/to/localmh/MathHub" $lmh_repo )
    fi
...
docker="$docker --tlsverify=false" # possible fix for KWARC/localmh#229
